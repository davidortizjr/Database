...
if (isset($_POST['btnEdit'])) {
    ...
    $editQuery = "UPDATE clients SET username = '$username', userInfoID = '$userInfoID', email = '$email', phoneNumber = '$phoneNumber' WHERE clientID = '$clientID'";
...
if (isset($_GET['editID'])) {
    $editID = $_GET['editID'];
    $editQuery = "SELECT * FROM clients WHERE clientID = '$editID'";
    $result = executeQuery($editQuery);
...
<body>
                            ...
                            <div class="card my-3">
                                <div class="card-body">
                                    ...
                                    <form method="get" class="d-inline">
                                        <input type="hidden" value="<?php echo $client['clientID']; ?>" name="editID">
    ...
    <section class="container my-5">
        <div class="card bg-dark text-light p-4 shadow">
            <h3 class="card-title mb-4">Edit Client Information</h3>
            <form method="post">
                <input type="hidden" name="clientID" value="<?php echo $editClient['clientID']; ?>">
                <div class="form-group mb-3">
                    <label for="username">Username</label>
                    <input type="text" id="username" name="username" class="form-control" value="<?php echo $editClient['username']; ?>" required>
                </div>
                <div class="form-group mb-3">
                    <label for="userInfoID">User Info ID</label>
                    <input type="text" id="userInfoID" name="userInfoID" class="form-control" value="<?php echo $editClient['userInfoID']; ?>" required>
                </div>
                <div class="form-group mb-3">
                    <label for="email">Email</label>
                    <input type="email" id="email" name="email" class="form-control" value="<?php echo $editClient['email']; ?>" required>
                </div>
                <div class="form-group mb-3">
                    <label for="phoneNumber">Phone Number</label>
                    <input type="text" id="phoneNumber" name="phoneNumber" class="form-control" value="<?php echo $editClient['phoneNumber']; ?>" required>
                </div>
